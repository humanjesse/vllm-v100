    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        if self.impl.supports_quant_query_input and self.kv_cache_dtype.startswith(
            ...
            # QuantFP8 currently uses the platform-default CUDA FP8 dtype.
            # On pre-SM89 GPUs that maps to e4m3, which is unavailable even if
            # the KV cache itself is configured as fp8_e5m2. In that case, skip
            # the optional query pre-quantization and let the backend consume
            # fp16 queries directly.
            if (
