...
@triton.autotune(
    configs=[
        # SM70 (V100): single config to avoid autotuner OOM.
        # NemotronH at TP=8: hdim=64, dstate=128.
        triton.Config(
...
@triton.autotune(
    configs=[
        triton.Config(
            # SM70: single config to avoid autotuner OOM
            {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 32},
            num_stages=4,
            num_warps=4,
        ),
...
def _chunk_state_fwd(
    ...
    if os.environ.get("VLLM_SSD_DEBUG"):
        ...
        print(f"[SSD_DEBUG] _chunk_state_fwd: seqlen={seqlen} nheads={nheads} "
              ...
              f"states_shape=({nchunks},{nheads},{headdim},{dstate})",
              flush=True)
